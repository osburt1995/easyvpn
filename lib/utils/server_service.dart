...
class ServerSercice{
  static setData(key, value) async {
     ...
     *  */
    try {
      List serversListData = json.decode(await Storage.getString(key));

      var hasData = serversListData.any((v) {
        return v == value;
      });
      if (!hasData) {
        serversListData.add(value);
        await Storage.setString(key, json.encode(serversListData));
      }
